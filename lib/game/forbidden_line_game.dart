...
class ForbiddenLineGame extends FlameGame
  ...
  // === سیستم صوتی حرفه‌ای (Audio Pools) ===
  // تعریف استخرهای جداگانه برای جلوگیری از تداخل و کرش
  ...
  Future<void> onLoad() async {
    ...
    // لود کردن موزیک پس‌زمینه و صدای باخت (تکی)
    await FlameAudio.audioCache.loadAll(['sfx/crash.wav', 'music/bgm.mp3']);

    // === مقداردهی استخرهای صوتی ===
    // minPlayers: حداقل تعداد آماده باش | maxPlayers: حداکثر پخش همزمان
    poolWhoosh = await FlameAudio.createPool(
      'sfx/whoosh.wav',
      minPlayers: 3,
      maxPlayers: 10,
    );
    poolTap = await FlameAudio.createPool(
      'sfx/tap.wav',
      minPlayers: 2,
      maxPlayers: 5,
    );
    poolCoin = await FlameAudio.createPool(
      'sfx/coin.wav',
      minPlayers: 3,
      maxPlayers: 10,
    );
    poolShield = await FlameAudio.createPool(
      'sfx/shield.wav',
      minPlayers: 1,
      maxPlayers: 3,
    );
    poolIce = await FlameAudio.createPool(
      'sfx/ice.wav',
      minPlayers: 1,
      maxPlayers: 3,
    );
  ...
  void render(Canvas canvas) {
    ...
    for (
      double i = (gridScrollOffset % gridSize) - gridSize;
      i < canvasSize.y;
      i += gridSize
    ) {
  ...
  void activateFreeze() {
    if (freezeCharges.value > 0 && !isActiveFreeze) {
      ...
      poolIce.start(volume: 1.0); // صدای یخ

      shakeCamera(intensity: 5.0);
      HapticFeedback.mediumImpact();

      // افکت موج یخ روی بازیکن
      world.add(
        ShockwaveEffect(
  ...
  void buyUpgrade(String type, int cost) {
    if (totalCoins >= cost) {
      ...
      if (type == 'freeze') {
        freezeLevel++;
        prefs.setInt('freeze_level', freezeLevel);
      } else if (type == 'coin') {
        coinLevel++;
        prefs.setInt('coin_level', coinLevel);
      } else if (type == 'shield') {
        shieldLevel++;
        prefs.setInt('shield_level', shieldLevel);
      }
  ...
  void buySkin(int skinId, int cost) {
    if (totalCoins >= cost && !ownedSkins.contains(skinId.toString())) {
      ...
      prefs.setInt('total_coins', totalCoins);
      prefs.setStringList('owned_skins', ownedSkins);
      poolTap.start(volume: 1.0);
      HapticFeedback.lightImpact();
    }
  }
  ...
  void equipSkin(int skinId) {
    if (ownedSkins.contains(skinId.toString())) {
      selectedSkinIndex = skinId;
      prefs.setInt('current_skin', selectedSkinIndex);
      player.updateSkin(skinId);
      poolTap.start(volume: 1.0);
  ...
  void update(double dt) {
    if (!isPlaying || isGameOver) {
      for (var child in world.children) {
        // آپدیت افکت‌ها حتی در زمان توقف برای زیبایی بصری
        if (child is DebrisParticle ||
            child is SparkleParticle ||
            child is ShockwaveEffect) {
          child.update(dt);
        }
      }
      super.update(0);
    ...
    if (shakeTimer > 0) {
      ...
      camera.viewfinder.position = Vector2(
        (_random.nextDouble() - 0.5) * shakeIntensity * 2,
        (_random.nextDouble() - 0.5) * shakeIntensity * 2,
      );
    } else if (camera.viewfinder.position.x != 0 ||
        camera.viewfinder.position.y != 0) {
      camera.viewfinder.position = Vector2.zero();
    }

    survivalTime += dt;
    if (survivalTime >= 60.0 && currentZone == 1) {
      currentZone = 2;
      triggerZone2();
    }
    ...
    for (var line in obstacles) {
      if (line.position.y > halfHeight + ForbiddenLine.lineHeight) {
        ...
        for (var otherLine in obstacles) {
          if (otherLine.position.y < highestY) highestY = otherLine.position.y;
        }
  ...
  void _spawnRandomPowerUp() {
    ...
    world.add(
      PowerUp(type: type, position: Vector2(randomX, (-gameHeight / 2) - 100)),
    );
  }
  ...
  void reviveGame() {
    ...
    for (var line in obstacles) {
      if ((line.position.y - player.position.y).abs() < 200) {
        line.position.y -= 500;
      }
    }
  ...
  void triggerNearMiss() {
    ...
    shakeCamera(intensity: 8.0);
    nearMissCount++;
    HapticFeedback.mediumImpact();

    int bonusPoints = nearMissCount <= 10
        ? nearMissCount
        : (nearMissCount <= 20 ? 5 : 10);
    rawScore += bonusPoints;
    world.add(
      BonusText(
        amount: bonusPoints,
        position: Vector2(player.position.x, player.position.y - 60),
      ),
    );
  ...
  void resetGame({bool restartMusic = false}) {
    ...
    // پاکسازی کامل صحنه
    world.children.whereType<PowerUp>().forEach(
      (powerUp) => powerUp.removeFromParent(),
    );
    world.children.whereType<TrailParticle>().forEach(
      (trail) => trail.removeFromParent(),
    );
    world.children.whereType<DebrisParticle>().forEach(
      (d) => d.removeFromParent(),
    );
    world.children.whereType<SparkleParticle>().forEach(
      (d) => d.removeFromParent(),
    );
    world.children.whereType<ShockwaveEffect>().forEach(
      (d) => d.removeFromParent(),
    );
    ...
    for (int i = 0; i < numberOfLines; i++) {
      obstacles[i].resetLine((-gameHeight / 2) - (i * lineSpacing));
    }
